app.factory('Logger', function($http) {
	...
	logger.login = function (username, password, ip, port) {
		...
		/*	Test de connexion, on va demander à Kodi de se déplacer vers la gauche	*/
		test_url = '/jsonrpc?request={"jsonrpc":"2.0","method":"Input.Left"}&callback=JSON_CALLBACK';
		$http.jsonp(window.base_url+test_url);
