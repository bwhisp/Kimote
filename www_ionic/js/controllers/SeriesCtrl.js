app.controller('SeriesCtrl', function($scope, $http, $location, $stateParams, $ionicLoading) {
    ...
    /*$scope.showSeriesDetails = function(tvshowid) {
        method = "VideoLibrary.GetTVShowDetails";
        params = '{"tvshowid":' + tvshowid + ', "properties": ["title"]},"id": "libTvShows"';

        getSeriesDetails($http, method, params);
    };

    function getSeriesDetails($http, method, params) {
		...
		$http.jsonp(complete_url, {params: {callback: 'JSON_CALLBACK', format: 'json'}})
		.success(function(data, status, headers, config) {
			$scope.tvshowdetails = data.result.tvshowdetails;
		})
		.error(function(data, status, headers, config) {
			console.log('Data: ' + data);
            console.log('Status: ' + status);
